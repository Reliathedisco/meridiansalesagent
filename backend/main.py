...
class RAGPipeline:
    ...
    def _build_body(self, query: str, context_chunks: list[dict]) -> dict:
        context = "\n\n---\n\n".join(
    ...
    async def generate(self, query: str, context_chunks: list[dict]) -> str:
        body = self._build_body(query, context_chunks)
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await client.post(ANTHROPIC_URL, headers=ANTHROPIC_HEADERS, json=body)
        data = resp.json()
    ...
    async def generate_stream(
        ...
        body = {**self._build_body(query, context_chunks), "stream": True}
        async with httpx.AsyncClient(timeout=60.0) as client:
            async with client.stream(
                ...
                if resp.status_code != 200:
                    ...
                    yield {"type": "error", "text": f"API error ({resp.status_code}): {msg}"}
                    return
...
async def chat(req: ChatRequest):
    ...
    async def event_stream():
        ...
        yield _sse({"type": "done", "chunks": chunks_meta})
...
# Local dev: serve the frontend HTML directly
